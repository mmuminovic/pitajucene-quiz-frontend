...
class User extends Component {
  ...
  componentDidMount() {
    if (
      this.props.user.userId === this.props.match.params.userId ||
      this.props.isAdmin
    ) {
      this.getUserInfo();
  ...
  getCurrentScore = () => {
    const userId = this.props.match.params.userId;
    axios.get(`/myscore/${userId}`).then((result) => {
  ...
  getLastMonthScore = () => {
    const userId = this.props.match.params.userId;
    axios.get(`/scorelastmonth/${userId}`).then((result) => {
      if (result.data.score) {
  ...
  getMyBestScore = () => {
    const userId = this.props.match.params.userId;
    axios.get(`/thebestscore/${userId}`).then((result) => {
      if (result.data.score) {
  ...
  editUser = () => {
    ...
    axios
      .patch(`/user/${this.props.match.params.userId}`, {
  ...
  changeWinnerStatus = () => {
    axios
      .patch(`/set-winner/${this.props.match.params.userId}`)
      .then((result) => {
  ...
  render() {
    ...
    if (this.state.quizRemaining) {
      quizRemaining = (
        <tr>
          <th
            colSpan="2"
            style={{ backgroundColor: "#E04836", padding: "10px" }}
          >
            Imate nezavršen kviz. Skrolajte na dole, pronađite ga i nastavite.
          </th>
        </tr>
      );
    }
    ...
    if (this.state.games) {
      if (this.state.games.length > 0) {
        let wrongAnswers = this.state.games.map((q, i) => {
          if (!q.timeIsUp) {
            ...
            return (
              <li
                key={i}
                style={{ backgroundColor: "rgba(68, 109, 136, 0.2)" }}
              >
                <p>{q.time}</p>
                <p className={classes.Question} style={{ color: "white" }}>
                  {q.questionText}
                </p>
                <div className={classes.Field}>
                  {this.props.match.params.userId === this.props.user.userId ? (
                    <Link to={link}>
                      <button
                        style={{
            ...
            return (
              <li key={i} style={{ backgroundColor: "rgba(224, 73, 56, 0.2)" }}>
                ...
                <p>
                  Tačan odgovor možete naći na linku:{" "}
                  <a
                    href={q.questionLink}
                    target="_blank"
                ...
                <span style={{ paddingTop: "15px" }}>{q.score}</span>
              </li>
            );
            ...
            return (
              <li key={i} style={{ backgroundColor: "rgba(76, 174, 79, 0.2)" }}>
                ...
                <p className={classes.Question}>
                  Osvojili ste maksimalan broj bodova. Čestitamo!
                </p>
                <span>Ostvareni rezultat:</span>
                <span>
                  <img
            ...
            return (
              <li
                key={i}
                style={{ backgroundColor: "rgba(87, 151, 188, 0.2)" }}
              >
                <p>{q.time}</p>
                <p className={classes.Question}>{q.questionText}</p>
                <span>Ostvareni rezultat:</span>
        ...
        games = <ul className={classes.List}>{wrongAnswers}</ul>;
      } else {
        games = (
          <p style={{ color: "red", fontWeight: "bold" }}>
            Niste još nijednom pogriješili.
          </p>
        );
      }
    }
      ...
      <div className={classes.Heading}>
        ...
        {!this.state.isWinner ? (
          <div>
            {this.props.user.isAdmin ? (
              <img
                ...
                onClick={this.changeWinnerStatus}
              />
            ) : null}
          </div>
        ) : (
          <div
            ...
            style={{ margin: "20px", padding: "0", height: "auto" }}
          >
            <img
              src={winnerIcon}
              alt="medal"
            ...
            <div>
              {this.props.user.isAdmin ? (
                <button onClick={this.changeWinnerStatus}>
                  Ukloni pobjednika
                </button>
              ) : null}
            </div>
          ...
          <tbody>
            <tr>
              <td style={{ padding: "5px 0" }}>
                Pogledajte pitanja na koja ste dali netačan odgovor i pronađite
                tačan odgovor na datom linku.
              </td>
            </tr>
          </tbody>
        </table>
