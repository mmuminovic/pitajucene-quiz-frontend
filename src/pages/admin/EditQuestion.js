...
import { editQuestion, addQuestion } from '../../services/questions';
...
const EditQuestion = (props) => {
  ...
  const [addNewQuestion] = useMutation(() => addQuestion(state), {
    onSuccess: (data) => {
      if (data.error) {
  ...
  const [updateQuestion] = useMutation(() => editQuestion(state), {
    onSuccess: (data) => {
      if (data.error) {
        alert(`Greška: ${data.error}`);
      } else {
